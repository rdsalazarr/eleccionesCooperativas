...
export default function Frm({id}) {
    ...
    const adicionarFila = () => {
        ...
        newJuradosAgencias.push({identificador: '', documento: documento, nombre: nombre, cargo: cargo, estado: 'I'});
    ...
    const eliminarFila = (id) =>{
        ...
        juradosAgencias.map((res,i) =>{
            if(res.estado === 'U' && i === id){
                newJuradosAgencias.push({ identificador:res.identificador, documento: res.documento, nombre:res.nombre, cargo: res.cargo, estado: 'D' });
            }else if(res.estado === 'D' && i === id){
                newJuradosAgencias.push({identificador:res.identificador, documento: res.documento, nombre:res.nombre, cargo: res.cargo, estado: 'U'});
            }else if((res.estado === 'D' || res.estado === 'U') && i !== id){
                newJuradosAgencias.push({identificador:res.identificador, documento: res.documento, nombre:res.nombre, cargo: res.cargo, estado:res.estado});
            }else{
                if(i != id){
                    newJuradosAgencias.push({identificador:res.identificador, documento: res.documento, nombre:res.nombre, cargo: res.cargo, estado: 'I' });
                }
            }
        })
        setJuradosAgencias(newJuradosAgencias);
    ...
    const onSubmit = (formValues) => {
        ...
        if(juradosAgencias.length === 0){
            ShowSnackbar('Debe adicionar como mínimo un jurado para la agencia', 'error');
            return
        }
    ...
    useEffect(()=>{
        setLoader(true);
        instance.post('/admin/eleccion/delegado/gestion/asignar/jurados/list', {codigo: id}).then(res=>{
            if(res.success){
                let newJuradosAgencias = [];
