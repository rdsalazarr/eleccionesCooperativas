...
export default function frm(){
    ...
    const { register, handleSubmit, watch, setValue, reset, setError, clearErrors, control, formState: { errors } } = useForm({
                        ...
                        defaultValues:{numeroDocumento: '1978917', token: '146',  candidatos: []},
                        mode: "onSubmit"
                    });
    ...
    const iniciar = () => {
        reset({ numeroDocumento: '1978917', token: '146',  candidatos: []});
        setTipoOrgano([]);
        setAspirantes([]);
        setDelegado([]);
        setActiveStep(0)
    ...
    const seleccionarCandidato = (idCandidato) => {
        ...
        const maximo = Number(tipoOrgano?.tiporgvotosporpersona || 0);
        ...
        // Si estaba seleccionado el voto en blanco,
        // se elimina para poder seleccionar candidatos
        let nuevosCandidatos = candidatosSeleccionados.filter(
            id => Number(id) !== Number(idVotoBlanco)
        );

        // Si ya estaba seleccionado, lo quitamos
        if (nuevosCandidatos.some(id => Number(id) === Number(idCandidato))) {
            nuevosCandidatos = nuevosCandidatos.filter(
                id => Number(id) !== Number(idCandidato)
            );
        } else {

            // Si ya llegó al máximo, no permite agregar más
            if (nuevosCandidatos.length >= maximo) {
                return;
            }
    ...
    const onSubmitVoto = (formValues) => {
       const candidatos = formValues.candidatos;
        if (candidatos.length === 0) {
            setError('candidato', {type: 'manual', message: 'Debe seleccionar como mínimo un candidato para este tipo de órgano o el voto en blanco' });
            return;
        }
                            ...
                            <Grid container spacing={3}>
                                <Grid size={{ xs: 12 }}>
                                    <Box className="listaCategoria">
                                        <Box className="listaCategoriaInfo">
                                                ...
                                                <Grid size={{ xs: 12, sm: 6 }} style={{textAlign:'right'}}>
                                                    <span className="tituloCategoria">
                                                        <Box style={{ display:'flex', alignItems:'center', gap:'8px'}}> 
                                                            <span className="numerosCandidatos"> {numeroCandidatosSeleccionados} </span> / <span>{tipoOrgano?.tiporgvotosporpersona}</span>
                                            ...
                                            <h2>
                                                Elige tus candidato a particiar del {tipoOrgano?.tiporgnombre}
                                            </h2>
                                            ...
                                            <p className="instruccionVoto">
                                                Selecciona hasta <b>{tipoOrgano?.tiporgvotosporpersona}</b> candidatos.
                                            </p>
                                        </Box>
                                    </Box>
                                </Grid>
                                ...
                                {aspirantes.map((res) => (
                                    <Grid
                                       ...
                                       <Box
                                            className={`candidato ${
                                                watch("candidatos")?.some(
                                                    id => Number(id) === Number(res.deleid)
                                                )
                                                    ? "selected"
                                                    : ""
                                            }`}
                                            onClick={() => seleccionarCandidato(res.deleid)}
                                        >
                                            ...
                                            <Box className="avatar">
                                                {res.rutaFoto ? (
                                                    ...
                                                    <span><img src={fotoDefault} /> {res.orelpaordenparticipacion}</span>
                                                )}
                                            </Box>
                        ...
                        <Grid container spacing={3} className="animate__animated animate__fadeInUp" >
                            <Grid size={{ xs: 12}}>
                                  <div className="mensajeVerificacion">
                                    <div className="mensajeVerificacionIcon">
                                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                        <circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/>
                                        </svg>
                                    </div>
                                    <h1>Proceso guardado con éxito</h1>
                                    <p >Tu selección para este órgano fue registrada correctamente.</p>
                                    <div className="tituloCategoria">
                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M20 6L9 17l-5-5"/></svg>
                                        <span id="savedOrgano"> {tipoOrgano?.tiporgnombre}</span>
                                    </div>
                                    <p>hora debes esperar a que el presidente cierre este órgano y habilite el siguiente..</p>
                                    <p>Esta pantalla se actualiza automáticamente pasado 10 segundos.</p>
                                    </div>
                            </Grid>
